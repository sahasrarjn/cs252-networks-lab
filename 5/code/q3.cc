...
void experiment (bool enableCtsRts, std::string wifiManager, std::string dr)
{
 ...
 //First create the loss matrix object called "lossModel"
  ...
  // set symmetric loss 0 <-> 1 to 50 dB, good coverage
    lossModel->SetLoss (nodes.Get (0)->GetObject<MobilityModel> (), nodes.Get (1)->GetObject<MobilityModel> (), 50);  // A1-A2
    lossModel->SetLoss (nodes.Get (0)->GetObject<MobilityModel> (), nodes.Get (2)->GetObject<MobilityModel> (), 50);  // A1-B1
    lossModel->SetLoss (nodes.Get (1)->GetObject<MobilityModel> (), nodes.Get (3)->GetObject<MobilityModel> (), 50);  // A2-B3
  
  // set symmetric loss 0 <-> 2 to 50 dB, good coverage
    lossModel->SetLoss (nodes.Get (2)->GetObject<MobilityModel> (), nodes.Get (3)->GetObject<MobilityModel> (), 50);  // B1-B2
 ...
 //With this statement we are assuming that 10.0.0.1 as the IP address of node "n0"
  ...
  std::string dataRate = dr; 
	double stime;
   
  //OnOff source can be set to just generate packets at some data rate)
  OnOffHelper onOffHelper1 ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address ("10.0.0.2"), cbrPort));
  onOffHelper1.SetConstantRate(DataRate (dataRate), payloadSize);
  

  // flow 1:  node 0 -> node 1. Set start time attributes  
  //Now Install this helper object on ni, and add the returned object, an application to the cbrApps container
	 ...
	 onOffHelper1.SetAttribute ("StartTime", TimeValue (Seconds (stime)));
  ...
  onOffHelper2.SetConstantRate(DataRate (dataRate), payloadSize);


  // flow 2:  node 2 -> node 3. Set start time attributes slightly different
  /** \internal
   ...
   */
	 ...
	 onOffHelper2.SetAttribute ("StartTime", TimeValue (Seconds (stime)));
	 cbrApps.Add (onOffHelper2.Install (nodes.Get (2)));



  //OnOff source can be set to just generate packets at some data rate)
  OnOffHelper onOffHelper3 ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address ("10.0.0.6"), cbrPort));
  onOffHelper3.SetConstantRate(DataRate (dataRate), payloadSize);

   // flow 3:  node 4 -> node 5. Set start time attributes slightly different
  /** \internal
   * The slightly different start times and data rates are a workaround
   * for \bugid{388} and \bugid{912}
   */
   ...
   onOffHelper3.SetAttribute ("StartTime", TimeValue (Seconds (stime)));
   cbrApps.Add (onOffHelper3.Install (nodes.Get (4)));
   ...
   */
  uint16_t  echoPort = 9;
  UdpEchoClientHelper echoClientHelper1 (Ipv4Address ("10.0.0.2"), echoPort);
  UdpEchoClientHelper echoClientHelper2 (Ipv4Address ("10.0.0.4"), echoPort);
  UdpEchoClientHelper echoClientHelper3 (Ipv4Address ("10.0.0.6"), echoPort);
  ...
  echoClientHelper1.SetAttribute ("StartTime", TimeValue (Seconds ((double)1/1000)));
  pingApps.Add (echoClientHelper1.Install (nodes.Get (0)));

echoClientHelper2.SetAttribute ("StartTime", TimeValue (Seconds ((double)2/1000)));
  pingApps.Add (echoClientHelper2.Install (nodes.Get (2)));

echoClientHelper3.SetAttribute ("StartTime", TimeValue (Seconds ((double)2/1000)));
  pingApps.Add (echoClientHelper3.Install (nodes.Get (4)));
...
int main (int argc, char **argv)
{
  ...
  for(int i=1; i<n; i++){
    ...
    double temp = ((double) i)*1.8;
    std::string drt = std::__cxx11::to_string(temp) + "Mbps";
    std::cout << drt << '\n';
    std::cout << "Hidden station experiment with RTS/CTS disabled:\n" << std::flush;
    experiment (false, wifiManager, drt);
    std::cout << "------------------------------------------------\n";
    std::cout << "Hidden station experiment with RTS/CTS enabled:\n";
    experiment (true, wifiManager, drt);
  }
